...
def f1w(tp):
    ...
    return (tp[tp.wk.isin(wks[-1:])]['当日收益率'].fillna(0)+1).cumprod().values[-1]-1
def f2w(tp):
    tp=tp[(tp.账户==tp.账户.values[-1])&(tp.策略==tp.策略.values[-1])]
    wks=list(tp.wk.unique())
    return (tp[tp.wk.isin(wks[-2:])]['当日收益率'].fillna(0)+1).cumprod().values[-1]-1
def fm(tp):
    tp=tp[(tp.账户==tp.账户.values[-1])&(tp.策略==tp.策略.values[-1])]
    return (tp[tp.mon==tp.mon.values[-1]]['当日收益率'].fillna(0)+1).cumprod().values[-1]-1
ret_2w=[]
ret_1m=[]
ret_1w=[]
...
df.loc[:,'本月收益']=ret_1m
df.loc[:,'本周收益']=ret_1w
df[['当日收益率','alpha','本周收益','近2周收益','本月收益']]=np.round(df[['当日收益率','alpha','本周收益','近2周收益','本月收益']]*100,1)
...
分析名称='test'
tp=df[df.日期==df.日期.max()][['日期','账户','策略','当日收益率','本周收益','近2周收益','本月收益']]
tp=tp.set_index(['日期','策略'])[['当日收益率','本周收益','近2周收益','本月收益']].stack().reset_index()
...
df[df.日期==df.日期.max()][['日期','账户','策略','当日收益率','本周收益','近2周收益','本月收益']]
